...
#ifdef _WIN32
int Thread_Create(TheoraDecoder *ctx, void *(*routine) (void*)) {
...
void Thread_Join(THEORAPLAY_THREAD_T thread) {
    WaitForSingleObject(thread, INFINITE);
    CloseHandle(thread);
}
int Mutex_Create(TheoraDecoder *ctx) {
	ctx->lock = CreateMutex(NULL, FALSE, NULL);
    return (ctx->lock == NULL);
}
void Mutex_Destroy(THEORAPLAY_MUTEX_T mutex) {
    CloseHandle(mutex);
}
void Mutex_Lock(THEORAPLAY_MUTEX_T mutex) {
    WaitForSingleObject(mutex, INFINITE);
}
void Mutex_Unlock(THEORAPLAY_MUTEX_T mutex) {
    ReleaseMutex(mutex);
}
#else
                                               ...
                                               THEORAPLAY_VideoFormat vidfmt)
{
	...
	byte *buffer = extractMem(fname, openPak("data.pak"), &size);
	if (buffer == NULL)
		return NULL;

	f = fmemopen (buffer, size, "rb");
#else
    f = fopen(fname, "rb");
#endif
    if (f == NULL)
